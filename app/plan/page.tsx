...
const missionSchema = z.object({
  ...
  altitude: z
    .number()
    .min(20, "Minimum altitude is 20m")
    .max(500, "Maximum altitude is 500m"),
  overlap: z
    .number()
    .min(50, "Minimum overlap is 50%")
    .max(90, "Maximum overlap is 90%"),
  speed: z
    .number()
    .min(5, "Minimum speed is 5 m/s")
    .max(20, "Maximum speed is 20 m/s"),
...
const generateCrosshatchPattern = (
  ...
  params: { altitude: number; overlap: number; speed: number }
) => {
...
const generatePerimeterPattern = (
  areaBounds: L.LatLngBounds,
  params: { altitude: number; overlap: number }
) => {
  const ne = areaBounds.getNorthEast();
  const sw = areaBounds.getSouthWest();
...
const calculateAreaCoverage = (areaGeoJSON: any, waypoints: any[]) => {
  ...
  const area =
    areaGeoJSON.coordinates[0].reduce(
      (acc: number, coord: [number, number], i: number) => {
        if (i === 0) return 0;
...
export default function MissionPlanning() {
  ...
  const [drones, setDrones] = useState<
    { id: string; model: string; status: string; battery: number }[]
  >([]);
  ...
  useEffect(() => {
    const fetchDrones = async () => {
      try {
        ...
        const availableDrones = data.drones.filter(
          (drone: any) => drone.status === "available" && drone.battery > 20
        );
    ...
    try {
      ...
      switch (watchedPattern) {
        ...
        case "spiral":
          ...
          const radiusStep =
            Math.min(
              bounds.getNorthEast().lat - center.lat,
              center.lng - bounds.getSouthWest().lng
            ) / 4;
          for (
            let r = radiusStep;
            r <
      ...
      toast.success(
        `Generated ${watchedPattern} pattern with ${generatedWaypoints.length} waypoints (${coverage}%)`
      );
    } catch (error) {
            ...
            <CardContent>
              <div className="relative">
                <MapContainer
                  ...
                  whenReady={() => setMap(mapRef.current)} // ← Wrap in a function
                  ref={mapRef}
                >
                  <TileLayer
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {drones.length === 0 && (
                        <p className="text-sm text-gray-500">
                          No drones available. All drones are either in mission
                    ...
                    <CardContent className="space-y-4 pt-0">
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-gray-500">
                          Recommended: 80-150m for optimal coverage
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        />
                        <div className="text-xs text-gray-500">
                          m/s - Balance between coverage and detail
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="perimeter">
                          <div className="flex items-center space-x-2">
                            <div className="w-4 h-4 border rounded text-xs">
                              □
                            </div>
                            <span>Perimeter</span>
                          </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {[
                        ...
                        <div
                          key={value}
                          className="flex items-center space-x-2 p-2 rounded-md border hover:bg-gray-50"
                        >
                          <Switch
                            id={value}
                  ...
                  <div className="space-y-2">
                    ...
                    <Select defaultValue="5s" {...register("frequency")}>
                      ...
                      <SelectContent>
                        <SelectItem value="2s">
                          Every 2 seconds (High resolution)
                        </SelectItem>
                        <SelectItem value="5s">
                          Every 5 seconds (Recommended)
                        </SelectItem>
                        <SelectItem value="10s">
                          Every 10 seconds (Fast survey)
                        </SelectItem>
                        <SelectItem value="30s">
                          Every 30 seconds (Quick overview)
                        </SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
              ...
              <CardContent className="space-y-2 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {drawnArea
                      ? `${drawnArea.geometry.coordinates[0].length} vertices`
                      : "Not defined"}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {waypoints.length
                      ? `${Math.round(waypoints.length * 0.5)} min`
                      : "N/A"}
                  </span>
                </div>
